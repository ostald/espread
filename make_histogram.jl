...
if !isdir(joinpath(dir, "hist_pitch"))
    mkdir(joinpath(dir, "hist_pitch"))
end
...
for file in dir_con_raw
    ...
    df.pitch = pitch_angle.(df.v0)
    df.pitch_primary = copy(df.pitch)
    ...
    for row in eachrow(df)
        if row.generation == 1
            ...
            row.pitch_primary = pitch
        end
    end

    pitch_edges = collect(0:1:180) ./ 180 .*pi
    z_edges = collect(80e3:1e3:600e3)

    df_primary = filter(:alt0 => x -> x > 599e3, df)
    ne_pitch = fit(Histogram, df_primary.pitch_primary, pitch_edges)


    his_pitch = fit(Histogram, ([p[3] for p in df.pos], df.pitch_primary), (z_edges, pitch_edges))
    open(joinpath(dir, "hist_pitch", "h_pitch_$(file[5:end-4]).hist"), "w") do io
        serialize(io, [E0, lim_pitch_deg, seed_value, hmin, hmax, hintervals, his_pitch, ne_pitch])
    end
...
if !isdir(joinpath(dir, "hist_pitch_summed"))
    mkdir(joinpath(dir, "hist_pitch_summed"))
end
dir_con = readdir(joinpath(dir, "hist_pitch"))
runs = unique([d[1:end-10] for d in dir_con])
#radial_runs = filter(x-> contains(x, "hrp"), runs)
#cartesian_runs = filter(x-> contains(x, "xyz"), runs)
...
for r in runs
    ...
    for file in list_h_run
        println("Processing histogram file: ", file)
        io = open(joinpath(dir, "hist_pitch", file), "r")
        E0, lim_pitch_deg, seed_value, hmin, hmax, hintervals, his_i, ne_pitch = deserialize(io)
        close(io)
    ...
    open(joinpath(dir, "hist_pitch_summed", r * "_summed.hist"), "w") do io
        serialize(io, [E0, lim_pitch_deg, seed_value, hmin, hmax, hintervals, his_summed, ne_pitch_summed])
    end
end
