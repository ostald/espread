...
function scatter_elastic_He(v_in, E_exc = 0)
    E_in = E_ev(norm(v_in))
    theta = scatter_angle_el_inel(E_in, phase_fcn_O, "elastic", 1) #use Oxigen phase function 
    #println(string(theta/pi*180))
    phi = rand()*2*pi
    E_out = E_in
    @assert E_out > 0
...
function scatter_inelastic_He(v_in, E_exc)
    E_in = E_ev(norm(v_in))
    theta = scatter_angle_el_inel(E_in, phase_fcn_O, "inelastic", 1) #use Oxigen phase function 
    phi = rand()*2*pi
    E_out = E_in - E_exc
    @assert E_out > 0
    v_out = scatter_particle(v_in, E_out, theta, phi)
...
function scatter_ion_He(v_in, E_ionization)
    E_primary = E_ev(norm(v_in))
    pdf, E_max = E_secondary_e_O(E_primary, E_ionization) #use oxigen secondaries
    Es_out = sample_secondary(pdf, E_max)
    #println(string(Es_out/E_primary), "  ", string(Es_out))
    Ep_out = E_primary - Es_out - E_ionization
    @assert Ep_out > 0
