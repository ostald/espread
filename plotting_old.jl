...
# 1. bare Counts
# 2. Production
# 3. altidute slice of production vs radial distance
...
scatterlines!(ax2, rg_array, 2 .* width_20 ./ rg_array ./sin(20/180*pi), color = Makie.wong_colors()[1],label = L"{\mathrm{field-aligned \, \theta_{lim} = 20^{\degree}}}")
errorbars!(ax2, rg_array, 2 .* width_20./ rg_array./sin(20/180*pi), err_width_20./ rg_array./sin(20/180*pi), color = Makie.wong_colors()[1])
scatterlines!(ax2, rg_array, 2 .* width_90./ rg_array, color = Makie.wong_colors()[2],label = "isotropic")
errorbars!(ax2, rg_array, 2 .* width_90./ rg_array, err_width_90./ rg_array, color = Makie.wong_colors()[2])
#display(fig)
#save(joinpath(dir, "plots", "width_h_int.png"), fig, px_per_unit = 3.3)
...
##
#linear fit to energy vs width plot
...
x = [0.1, 33]*1e3
lines!(ax, sqrt.(x/1e3), linear(sqrt.(x), fit_90.param))
lines!(ax, sqrt.(x/1e3), linear(sqrt.(x), fit_20.param))

#display(fig)
#____________________________________________________________________________________________________
##
#some experiments to find the unceratinty in width 
#conclusion: its very small
# the 1/e decay from the maximum is probably more problematic due to tying it the maximum
# both because of the position and height of the maximum.
...
##
#____________________________________________________________________________________________________
# countours for width plot
# all energies in one plot
# two plots for isotropic/field aligned
...
##
#____________________________________________________________________________________________________
#panel plot of width (form contours)
# both isotropic and field aligned in each panel
# 4 panels for different energies
...
##
#____________________________________________________________________________________________________
# bjorns requrested plot on the differences/ratio in produciton 
# field aligned / isotropic
...
# something with 75th percentile
...
##
#____________________________________________________________________________________________________
# production at specific altitudes:
